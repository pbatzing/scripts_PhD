...
const char* includePath="-I. -I$ROOTSYS/include -I$ALICE_PHYSICS/lib -I$ALICE_PHYSICS/include -I$ALICE_ROOT/include";
...
bool AliCodeNode::HasSourceParent()
{
  ...
  TString namet(GetName());
  namet.ReplaceAll(".h", ".cxx");
  TIter parents(&fParents);
  TObject* obj=NULL;
  while ((obj=parents())) {
    if (namet.CompareTo(obj->GetName())==0)
      return true;
  }
  return false;
}
...
int AliCodeTree::LoadClasses(TString& libs)
{
  ...
  while ((obj=next())!=NULL) {
	...
	continue;
      ...
      TString command;
      TString resfilename(gSystem->TempDirectory()); resfilename+="/findlib.txt";
      command.Form("for lib in $ALICE_ROOT/lib/*/lib*.so; do (nm $lib | grep %s | grep ' T ' | grep Class_Name > /dev/null) && echo $lib > %s; done", names.Data(), resfilename.Data());
      gSystem->Exec(command);
      ifstream resfile(resfilename.Data());
      if (resfile.good()) {
	TString result;
	if (!result.ReadLine(resfile).eof()) {
	  ...
	  while ((haveSlash=result.First('/'))>=0) result.Replace(0, haveSlash+1, "");
	  if (!libs.Contains(result)) {
	    cout << "loading dependency library '" << result << "' for class '" << names << "'" << endl;
	    gSystem->Load(result);
	    if (!libs.IsNull()) libs+=" ";
	    libs+=result;
	  }
