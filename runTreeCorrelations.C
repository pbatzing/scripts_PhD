...
#include "TPRegexp.h"
using namespace std;
void runTreeCorrelations(const char* options = "",int BinVersion = 1)
{
  bool isPbPb = false;
  bool ispp   = false;
  TString option = TString(options);
  if(option.Contains("pp")||option.Contains("PP")) 	ispp 	= true;
  if(option.Contains("PbPb")||option.Contains("PBPB")) 	isPbPb 	= true;
  if((!(isPbPb||ispp))||(isPbPb&&ispp)){cout << "Please choose either pp or PbPb as option." <<endl;return;}
  ...
  gROOT->LoadMacro(Form("%sTreeCorrelations.cxx+g",basedir.Data()));
//   gROOT->LoadMacro(Form("%sTaskConfig.C+g",basedir.Data()));
...
//   TaskConfig();
  ...
  Double_t MinTriggerPt = 8.0;
  Double_t MaxTriggerPt = 16.0;
  Double_t MinAssociatedPt = 4.0;
  Double_t MaxAssociatedPt = 8.0;
  ...
  ThreePartTreeCorrelations * task = new ThreePartTreeCorrelations(fname,"");
  task->SetListName(tname);
